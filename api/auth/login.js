...
passport.deserializeUser(function(user, cb) {
  ...
  passport.use(new LocalStrategy(async function verify(username, password, cb) {
    ...
    const isPasswordCorrect = await bcrypt.compare(password, "$2a$10$3mmelGQHBKAnMJeSSm/oye5xgcwrxVoon5sV5u7Yhtdnk16sYue8i");
    if(isPasswordCorrect){
        const isUsernameCorrect = await bcrypt.compare(username.trim().toLowerCase(), "$2a$10$DeSUvjlBn2VWeuinP7ZkGextq5WokuRFifSsCZSlSUsIc6bnjQuqa");
